...
import print_utils, test_time_augmentation
...
def tratarproblemas(areas, id, trazas_sin_salir):
    global alarma
    global cantidad_frames_analizados
    ...
    if slope > valorpendientemaxima or (lastAreaDif > 1.5 * areas[len(areas) - (2+int(trazas_sin_salir))] and lastAreaDif > 25000):
        ...
        print("¿Valor de pendiente en los últimos 5 frames analizados muy elevada?")
        if slope > valorpendientemaxima:
            print("Sí, valor de la pendiente: " + str(slope))
        else:
            print("No")
        
        print("¿Incremento brusco?")
        if lastAreaDif > areas[len(areas) - (2+int(trazas_sin_salir))]:
            print("Sí, último valor del área: " + str(areas[len(areas) - (2+int(trazas_sin_salir))]) + ". Diferencia con el nuevo área: " + str(lastAreaDif))
        else:
            print("No")
    ...
    plt.xlabel('Frame')
    plt.ylabel('Areas')
    plt.title('Grafica_objeto_' + str(id + 1))

    plt.savefig(r"C:\Users\Javi\Desktop\UNIVERSIDAD\TFG\output\graficas\Grafica_objeto_" + str(id + 1) + ".png")
    plt.close()
    ...
    plt.plot(x, y, 'o', y)
    plt.ylim(0, limite_y)
    #plt.xlim(0,cantidad_frames_analizados)
    plt.xlabel('Frame')
    plt.ylabel('Areas')
    plt.title('Grafica_objeto_' + str(id + 1) + '_Completa')

    plt.savefig(r"C:\Users\Javi\Desktop\UNIVERSIDAD\TFG\output\graficas\Grafica_objeto_" + str(id + 1) + "_Completa.png")
    plt.close()
...
def main(args):
    ...
    global areas
    global cantidad_frames_analizados
    frames_analizados = 20      #si =20, analizaremos uno de cada 20 (%20)
    ...
    if not os.path.isdir(os.path.dirname(args.output)):                                         #Creamos si no existe la dirección de salida
    ...
    trazas = np.zeros((2,7))            #[CentroideX       CentroideY      Clase       Frame_analizado      Frames_sin_aparecer       Area     id]
    
    #trazasnuevas = []                  #[CentroideX       CentroideY      Clase       Area                 id]
    
    iterador = 1
    #cantidad_frames_analizados = int(len(list(image_generator))/20)
